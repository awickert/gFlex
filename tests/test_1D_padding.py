...
Unit tests
----------
_recommended_pad_width_1d  — positive integer, scales with Te and n_wavelengths
_smooth_pad_Te_1d          — shape, inner domain preserved, outer edge, taper
                             monotonicity, error conditions
_pad_domain_1d             — shapes, inner domain, zero-padded load ring
...
from gflex.f1d import F1D, _pad_domain_1d, _recommended_pad_width_1d, _smooth_pad_Te_1d
...
# ---------------------------------------------------------------------------
# _recommended_pad_width_1d
# ---------------------------------------------------------------------------

def test_recommended_pad_width_1d_type_and_sign():
    p = _recommended_pad_width_1d(Te=35e3, dx=5000.0)
    assert isinstance(p, int) and p > 0


def test_recommended_pad_width_1d_scales_with_Te():
    """Stiffer plate → longer wavelength → wider padding."""
    p_thin = _recommended_pad_width_1d(Te=20e3, dx=5000.0)
    p_thick = _recommended_pad_width_1d(Te=50e3, dx=5000.0)
    assert p_thick > p_thin


def test_recommended_pad_width_1d_scales_with_n_wavelengths():
    p1 = _recommended_pad_width_1d(Te=35e3, dx=5000.0, n_wavelengths=1.0)
    p2 = _recommended_pad_width_1d(Te=35e3, dx=5000.0, n_wavelengths=2.0)
    assert p2 > p1


def test_recommended_pad_width_1d_decreases_with_dx():
    """Finer grid → same wavelength requires more cells."""
    p_coarse = _recommended_pad_width_1d(Te=35e3, dx=10000.0)
    p_fine = _recommended_pad_width_1d(Te=35e3, dx=5000.0)
...
def test_smooth_pad_Te_1d_shape():
    ...
    p = 5
    Te_pad = _smooth_pad_Te_1d(Te_arr, pad_width=p)
...
def test_smooth_pad_Te_1d_outer_edge_equals_Te_out():
    ...
    Te_out = 20e3
    p = 5
    Te_pad = _smooth_pad_Te_1d(Te_arr, pad_width=p, Te_out=Te_out)
...
def test_smooth_pad_Te_1d_default_Te_out_is_mean():
    ...
    Te_pad = _smooth_pad_Te_1d(Te_arr, pad_width=3)
...
def test_pad_domain_1d_shapes():
    ...
    qs = np.ones(nx)
    Te_pad, qs_pad, p = _pad_domain_1d(Te_arr, qs, dx=dx)
    assert isinstance(p, int) and p > 0
...
def test_pad_domain_1d_improves_fd_accuracy():
    ...
    the flexural forebulge and distorts the deflection.  _pad_domain_1d pushes
