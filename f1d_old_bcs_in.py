...
class F1D(Flexure):
  ...
  def coeff_matrix(self,D,drho,dx4,nu=0.25,g=9.8):
    ...
    # Try to have constant c0[i]
#    l2[i] = 0
#    l1[i] = 0
#    c0[i] = c0[i]
#    r1[i] = (Ddx4 * -1) * (c0[i]/(Ddx4 * 4) + drho*g)
#    r2[i] = 0
    ...
    Ddx4 = D[i+2]/dx4
    r2[i] = 0
    r1[i] = Ddx4 * -6 + (1/dx4) * (D[i] - 2*D[i+1] + D[i+2])
    c0[i] = (Ddx4 * 6) + (-2/dx4) * (D[i] - 2*D[i+1] + D[i+2]) + drho*g
    l1[i] = Ddx4 * -2 + (1/dx4) * (D[i] - 2*D[i+1] + D[i+2])
    l2[i] = Ddx4 * -1
    ...
    # Try to have constant c0[i]
#    l2[i] = 0
#    l1[i] = (Ddx4 * -6 + (1/dx4) * (D[i] - 2*D[i+1] + D[i+2])) / c0[i]
#    c0[i] = (Ddx4 * 6) + (-2/dx4) * (D[i] - 2*D[i+1] + D[i+2]) + drho*g
#    r1[i] = Ddx4 * -2 + (1/dx4) * (D[i] - 2*D[i+1] + D[i+2])
#    r2[i] = Ddx4 * -1
    ...
    # Boundary conditions
    # I think this makes a broken-ish plate...?
    for i in 0,1:
      Ddx4 = D[i+1]/dx4
      l2[i] = 0
      l1[i] = 0
    ...
    # Need to find out locations of l,r: now satisfied that l is on left
    #l2[0] = 1
    #cc[0,-2] = -l2[0]
    #cc[0,-1] = -l1[0]
    #cc[1,-1] = -l2[1]
...
#    if self.BC == 'Mirror':
    ...
    """
      ...
      for item in l2,l1,c0,r1,r2:
        item *= D/dx4
#      coeffs = vstack((r1,r2,l2,l1,c0,r1,r2,l2,l1))
      c0[0] += c0[0]
      r1[0] += l1[0]
      r2[0] += l2[0]
      l1[1] += l1[1]
